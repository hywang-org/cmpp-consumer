...
public class Cmpp20DeliverRequestMessageCodec extends MessageToMessageCodec<Message, CmppDeliverRequestMessage> {
	...
	protected void decode(ChannelHandlerContext ctx, Message msg, List<Object> out) throws Exception {
		...
		CmppDeliverRequestSelfDefinedMessage cmppDeliverRequestSelfDefinedMessage = new CmppDeliverRequestSelfDefinedMessage();
		cmppDeliverRequestSelfDefinedMessage.setHeader(msg.getHeader());
		cmppDeliverRequestSelfDefinedMessage.setBodyBuffer(msg.getBodyBuffer());
		requestMessage.setCmppDeliverRequestSelfDefinedMessage(cmppDeliverRequestSelfDefinedMessage);
		...
		if (registeredDelivery == 0) {
			...
			if (frameLength != Cmpp20ReportRequest.DESTTERMINALID.getBodyLength()) {
				logger.warn("CmppDeliverRequestMessage20 - MsgContent length is {}. should be {}.", frameLength,
	...
	protected void encode(ChannelHandlerContext ctx, CmppDeliverRequestMessage requestMessage, List<Object> out)
		...
		ByteBuf bodyBuffer = ctx.alloc()
				.buffer(Cmpp20DeliverRequest.DESTID.getBodyLength() + requestMessage.getMsgLength());
		...
		if (!requestMessage.isReport()) {
			...
			bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getReportRequestMessage().getStat()
					.getBytes(GlobalConstance.defaultTransportCharset), Cmpp20ReportRequest.STAT.getLength(), 0));
			bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getReportRequestMessage().getSubmitTime()
					.getBytes(GlobalConstance.defaultTransportCharset), Cmpp20ReportRequest.SUBMITTIME.getLength(), 0));
			bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getReportRequestMessage().getDoneTime()
					.getBytes(GlobalConstance.defaultTransportCharset), Cmpp20ReportRequest.DONETIME.getLength(), 0));
			bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(
		...
		bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(
				requestMessage.getReserved().getBytes(GlobalConstance.defaultTransportCharset),
