...
public class CmppSubmitRequestMessage extends DefaultMessage implements LongSMSMessage<CmppSubmitRequestMessage> {
	...
	public String getMsgContent() {
		if (msg instanceof SmsMessage) {
			System.out.println("here1 = " + msg.toString());
			return msg.toString();
		}

		if (msgContentBytes != null && msgContentBytes.length > 0) {
			LongMessageFrame frame = generateFrame();
			System.out.println("here2 = " + LongMessageFrameHolder.INS.getPartTextMsg(frame));
	...
	public CmppSubmitRequestMessage generateMessage(LongMessageFrame frame) throws Exception {
		...
		if (frame.getPknumber() != 1) {
			long sId = DefaultSequenceNumberUtil.getSequenceNo();
			System.out.println("generateMessage sId = " + sId);
			requestMessage.getHeader().setSequenceId(1163062742);
		}
		System.out.println("before generateMessage = " + requestMessage.getMsgContent());
		requestMessage.setMsg(null);
		System.out.println("after generateMessage = " + requestMessage.getMsgContent());
